...
struct hunterView {
    ...
    int score;   
    int roundNum;
    // ===== We can do these three in the newHunterView func at creation. Or jsut store the history String
    // ===== and do it when the respective functions are called. Depends on what seems easier.
    // ===== We're gonna be parsing the history string to get score, currentTurn and round anyway. Could
